            ...
            using (var client = new HttpClient())
            {
                ...
                if (minimalPermissions.Any())
                {
                    ...
                    if (string.IsNullOrEmpty(_configuration.FilePath))
                    {
                        ...
                        {
                            Logger.LogWarning("The following permissions are unnecessary: {permissions}", excessPermissions);
                        }
                        else
                        {
                ...
                if (errors.Any())
                {
                    Logger.LogError("Couldn't determine minimal permissions for the following URLs: {errors}", errors);
                }
            }
        }
        catch (Exception ex)
